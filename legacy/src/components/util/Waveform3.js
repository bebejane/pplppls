...
import './Waveform3.css';

class Waveform3 extends Component {
    constructor(props) {
        super(props);
        this.state = {
            id:props.id,
    ...
    onChange(duration){
        ...
        this.updateSize(()=>{
            this.setState({duration:duration, updating:true}, ()=>{
                setTimeout(()=>{
                    this.updateWaveform(true)
    ...
    updateWaveform(update, opt = {}){        
        const {id, bits, color, bgcolor, sampleRate,rate} = this.state;
        const duration = (opt.start || opt.end ? opt.end - opt.start : this.state.duration)*((1.0-rate)+1.0)
        const width = this.refContainer.current.clientWidth
        const height = this.refContainer.current.clientHeight
        const spp = Math.floor((duration*sampleRate)/width)
        console.log('WAVERATE', duration, rate)
