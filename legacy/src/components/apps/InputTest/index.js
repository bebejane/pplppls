...
class InputTest extends Component {
	...
	record(ms = 5000) {
		...
		Global.engine.on('state' + id, (state, updated) => {
			...
			pads[id] = state;
			this.setState({ pads });
			if (updated.playing) this.setState({ selected: id });
		});
			...
			Global.engine
				...
				.then((recording) => {
					Global.engine.on('load' + id, (on, i) => {
						...
						if (reverse) Global.engine.reverse(id, true);
	...
	download(recording) {
		Global.engine
			.encodeAudio(recording.buffer, 'wav', { numChannels: 1, sampleRate: 44100 })
			.then((data) => {
				console.log(data);
		...
		return new Promise((resolve, reject) => {
			...
			zip.loadAsync(zipContent).then((z) => {
				...
				Object.keys(z.files).forEach((k) => {
					if (z.files[k].name === 'index.json') promises.push(z.files[k].async('text'));
					else promises.push(z.files[k].async('arraybuffer'));
				});
				console.log('Decompressing', zipFile);
				Promise.all(promises)
					.then((result) => {
						for (var i = 0; i < result.length; i++) {
							if (typeof result[i] === 'string') model = JSON.parse(result[i]);
							else
					...
					<div className={'input-middle'}>
						<div>
							<div
								id='play'
								onClick={() => {
									!sampling ? this.record() : this.stop();
								}}
							>
								{!sampling ? 'REC' : 'STOP'}
							</div>
						...
						<div id='paddy-container'>
							{Object.keys(pads)
								.filter((k) => pads[k].loaded)
								.map((k, idx) => (
									<div
										key={idx}
										className={'paddy'}
										style={{
											backgroundColor: pads[k].playing
						...
						<div id='inputtest-tools'>
							<Switch
								onChange={(val, e) => this.setOption('reverse', val, e)}
								checked={options.reverse}
							/>
							<div>REV</div>
							<Switch
								onChange={(val, e) => this.setOption('loop', val, e)}
