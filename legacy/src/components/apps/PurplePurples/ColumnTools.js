...
class ColumnTools extends Component {
	...
	onMidiMapMode(on) {
		...
		if (unmap) return Global.engine.unmapMidiNote(this.state.id, this.state.midiNote);
	...
	render() {
		...
		return (
			...
			>
				{playing ? (
					<MdStop
						className={'sound-canvas-point-tool-toggle'}
						onClick={() => this.props.onStop()}
					/>
				) : (
					<MdPlayArrow onClick={() => this.props.onPlay()} />
				)}
				{!sampling ? (
					<MdFiberManualRecord
						onClick={() => this.onSampleRecord(true)}
						className={isSampling ? 'sound-canvas-point-tool-toggle' : undefined}
					/>
				) : (
					<MdFiberManualRecord
						className={
				...
				<MdVolumeUp
					className={!muted ? 'sound-canvas-point-tool-toggle' : undefined}
					onClick={() => this.props.onMute(!muted)}
				/>
				<MdRepeat
					onClick={() => this.props.onLoop(!loop)}
					className={loop ? 'sound-canvas-point-tool-toggle' : undefined}
				/>
				<RiArrowGoBackLine
					className={reversed ? 'sound-canvas-point-tool-toggle' : undefined}
					onClick={() => this.props.onReverse(!reversed)}
				/>
				<GiMagicLamp
					className={effectsEnabled ? 'sound-canvas-point-tool-toggle' : undefined}
					onClick={() => this.props.onEffectsEnabled(!effectsEnabled)}
				/>
				<AiOutlineLink
					className={
						midiNote
							? 'sound-canvas-point-tool-mapped'
							: midiMapMode
								? 'sound-canvas-point-tool-mapping'
								: undefined
					}
					onClick={() => this.onMidiMapMode(!midiMapMode)}
				/>
				<MdSettingsBackupRestore onClick={() => this.onReset()} />
				<AiFillPhone
					className={solo ? 'sound-canvas-point-tool-toggle' : undefined}
					onClick={() => this.props.onSolo(!solo)}
				/>
				<TiWaves
					className={fullscreen ? 'sound-canvas-point-tool-toggle' : undefined}
					onClick={() => this.onFullscreen(!fullscreen)}
				/>
				<IoMdLock
					className={locked ? 'sound-canvas-point-tool-toggle' : undefined}
					onClick={() => this.props.onLocked(!locked)}
				/>
