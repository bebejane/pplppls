...
class Sound extends EventEmitter {
  constructor(id, url, engine, opt = {filename:null, local:false, enableLoops:false, enableMeter:false, enableElapsed:false}) {
    super();
    ...
    this.node.connect(this.engine.masterGain);
    this.onEnded = this.onEnded.bind(this);
  }
   play(opt = {start:0, duration:0, enableElapsed:false}) {
    ...
    if(!this._muted && (soloOn ? this._solo : true)){
        const effects = this.effects.filter((e)=> !e.bypassed)
        effects.forEach((e)=>{
          this.source.connect(e.effect.inputNode);
          e.effect.connect(this.node)
        })
        // /if(!effects.length)
    ...
    this.node.gain.setValueAtTime(0, ct)
    this.source.start(0, this._offset, opt.duration !== undefined && !this._loop ? opt.duration : this._duration-this._offset);
    this.node.gain.linearRampToValueAtTime(opt.volume || this._volume, ct + 0.005);
    ...
    console.log('play', this._offset, 'muted', this._muted, 'loop', this._loopStart + ' > ' + this._loopEnd)
  ...
  _checkElapsed(){
        ...
        //console.log(this._elapsed + '/' + this._duration, this.context.currentTime, this._startedAt, this._offset)  
  ...
  stop() {
    ...
    if(!this.spillOver)
      this.effects.filter((e)=> !e.bypassed).forEach((e)=>e.effect.disconnect())
  ...
  _disconnectEffects(){
    const effects = this.effects.filter((e)=> !e.bypassed && e.type === 'pitchshift')
    effects.forEach((e)=>{
      e.effect.disconnect()
    })
  }
  ...
  pause(on) {
    if (on) {
      console.log('paused at', this._pausedAt)
      if(this.source){
      ...
      this._paused = false
      if(this._pausedAt){
      console.log('unpaused at', this._pausedAt)

        this.play({start:this._pausedAt})
      }
      this._pausedAt = 0
  ...
  jump(sec){
    ...
    this.pause()
    this.play({start:nextTime >= 0 ? nextTime : 0})
  }
  mute(on) {
    //if(this._muted === on) return this._emit('muted', on)
  ...
  loop(on, offset = {}){
    ...
    this._loopStart = offset.start !== undefined ? offset.start :  0
    this._loopEnd = offset.end !== undefined ? offset.end : on ? this._duration : 0
    this._loop = on
  ...
  crop(start, end){
    ...
    console.log('crop', s, e, data.length)
    const cropped = AudioUtils.slice([data], s, e > data.length-1 ? data.length-1 : e)
    const newBuff = new AudioBuffer({length:cropped[0].length, numberOfChannels:1, sampleRate:this.sampleRate})
  ...
  async pitch(pitch, opt) {
    ...
    if(pitch !== undefined && this.buffer){
        ...
        console.log('pitch it', pitch)
        const stretched = AudioUtils.stretch(this._org_buffer, {pitch:pitch, sampleRate:this.sampleRate})
        if(stretched.length){
          const newBuff = new AudioBuffer({length:stretched.length, numberOfChannels:1, sampleRate:this.sampleRate})
        ...
        */
    }else if(this._org_buffer)
        this.buffer = this._org_buffer
  ...
  addEffect(type, eff, enable){
    console.log('ADDding effect soundjs', this.effects.length)
    const idx = this.effects.length;
    const effect = {id:type, type:type, effect:eff, bypassed:true, idx:idx, defaults:eff.defaults}
    this.effects.push(effect)
    this._emit('addeffect', type)
    if(enable)
      this.effectBypass(idx, false)

    return this._currentEffectParams(idx)
  }
  removeEffect(idx){
    this.effects[idx].effect.disconnect()
  ...
  moveEffect(id, idx, fromIdx, toIdx){
    ...
    effects.forEach((e)=>{
        if(this.source && !e.bypassed){
            this.source.disconnect(e.effect.inputNode);
            e.effect.disconnect(this.node)
        }
    })
    this.effects = arrayMove(effects, fromIdx, toIdx)
    this.effects.filter((e)=>!e.bypassed).forEach((e)=>{
        if(this.source){
            this.source.connect(e.effect.inputNode);
            e.effect.connect(this.node)
        }
  ...
  effectBypass(idx, on){
    ...
    e.bypassed = on
    console.log((on ? 'BYPASS' : 'ACTIVATE') + ' EFFECT: ', this.id, idx)
    this._emit('effectbypass', idx, on)
    return e
  }
  ...
  disableEffects(){
    this.effects.forEach((e, idx)=>this.effectBypass(idx, true))
    this._effectsEnabled = false
    this._emit('effectsenabled', false)
  }
  enableEffects(){
    this.effects.forEach((e, idx)=>this.effectBypass(idx, false))
  ...
  load(url) {
    ...
    if (/^data:[^;]+;base64,/.test(url)) {
      console.log('decode Bas64', this._url)
  ...
  _loadLocal(){
      ...
      if(fs.existsSync(filePath)){
        const data = fs.readFileSync(filePath, {binary:true})
