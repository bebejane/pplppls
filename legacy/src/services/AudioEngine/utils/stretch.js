...
const stretch = (buffer, opt= {sampleRate:44100, pitc:1.0})=>{
    const stretcher = new TimeStretcher()
    const factor = 1.0 * opt.pitch;
    const data = buffer.getChannelData(0)
    stretcher.init({stretchFactor:factor, stftBins:2048, vocode:false})
    stretcher.setBuffer(data)
    stretcher.stretch()
    
    return stretcher.resize(data.length).getPitchShiftedBuffer()
}
const tools = () => {
