const tools = {
  ...
  L:(args) =>{
      ...
      $("#progress").text(Array.prototype.slice.caltools.L(arguments).join(" "));
      */
      //console.log(Array.prototype.slice.caltools.L(args.join(" "));
  },
  ...
  onProgress:(details, message)=>{
    ...
    if (details.complete) {
      ...
      if ((this.last_pct != progress_pct) && (progress_pct % 10 == 0)) {
        ...
        if (details.total_stages == 0) {
          tools.L(message, "Progress:", details.current_window,
            "/", details.total_windows, "("+progress_pct+"%)");
            this.last_pct = progress_pct;
        } else {
          tools.L(message, "Stage:", details.current_stage,
  ...
  interpolateArray:(data, newData, fitCount)=>{
    ...
    for ( var i = 1; i < fitCount - 1; i++) {
      ...
      tools.onProgress({current_stage: 1, total_stages: 0, current_window: i, total_windows: (fitCount - 1)},
        "Interpolating: ");
    }

    tools.onProgress({complete: true}, "Interpolating: ");
