...
class AudioEngine extends EventEmitter {
	constructor(
		...
		this.masterGain =
			typeof this.context.createGain === undefined
		...
		this.master = {
			...
			muted: () => {
				if (this.sounds.filter((s) => !s.sound.muted).length) return false;
				else return true;
			},
			pause: (on) => {
			...
			loop: (on) => {
				if (on === undefined) return this.master.state.looping;
			...
			_checkElapsed: () => {
				if (!this.master.state.playing) return this.master._clearElapsed();
	...
	rate(id, rate) {
		if (id) this.get(id).sound.rate(rate);
		else this.get().forEach((s) => s.sound.rate(rate));
	}
	pitch(id, pitch) {
		if (id) this.get(id).sound.pitch(pitch);
		else this.get().forEach((s) => s.sound.pitch(pitch));
	}
	mute(id, on = true) {
		if (id) {
