...
class Waveform extends Component {
	...
	componentDidUpdate(prevProps) {
		if (!this.state.init && !this.initing && this.state.duration) this.init();
		/*
	...
	init() {
		this.log('start init');
		if (!Global.engine) return console.log('waveform', 'engein not ready');
		if (this.state.init || this.state.initing) return this.log('already initing');
		...
		if (loopStart || loopEnd) {
			const selection = {
				start: parseInt((loopStart / duration) * width),
				end: parseInt((loopEnd / duration) * width),
			};
	...
	onState(state, updated) {
		if (updated.loop !== undefined && !updated.loop) this.resetSelection();
	}
	updateSize(cb) {
		if (!this.refContainer.current) return console.error('not ready');
		const dimensions = {
	...
	updateWaveform(update, opt = {}) {
		...
		if (!this._waveformData) return console.error('waveform no data!');
		...
		for (let i = 0, x = 0; i < left.length + blockSize; i += blockSize, x++) {
			...
			this.ctx.fillStyle = color;
			if (avg === 0 || avg === -0) this.ctx.fillRect(x, center, 1, 1);
			else {
	...
	handleMouseEvents(e) {
		...
		if (type === 'mousemove') this.setState({ position: x });
		if (type === 'mouseenter') {
		...
		if (selection.moveActive) return this.handleMove(e);
		...
		if (type === 'mousedown') {
			...
			if (this.props.onSelectionStart) this.props.onSelectionStart();
		} else if (type === 'mouseup') {
			selection.active = false;
			selection.handleActive = false;
			this.newSelection(selection);
		} else if (type === 'mousemove') {
			if (selection.active && !selection.handleActive) selection.end = x;
			else if (!selection.active && selection.handleActive === 'right') selection.end = x;
			else if (!selection.active && selection.handleActive === 'left') selection.start = x;
		} else return;

		if (_selection.start === selection.start && _selection.end === selection.end) return;
	...
	handleMouseClickOutside(e) {
		...
		if (selection.active || selection.handleActive || selection.movingActive) {
			if (e.type === 'mouseup') {
				if (selection.leaveLeft) selection.start = 0;
				else if (selection.leaveRight) selection.end = width;
				selection.handleActive = false;
				selection.active = false;
				//this.updateSelection(selection)
				this.newSelection(selection);
		...
		if (selection.start !== selection.end) {
			if (this.props.onSelection)
				this.props.onSelection({
					start: (selection.x / width) * duration,
					end: ((selection.x + selection.width) / width) * duration,
					time: (selection.x / width) * duration,
				});
		} else {
			this.resetSelection();
			if (this.props.onSelection)
	...
	updateSelection(selection, noCallback) {
		...
		if (this.props.onSelectionChange && !noCallback)
			this.props.onSelectionChange({
				start: (selection.x / selection.width) * duration,
				end: ((selection.x + selection.width) / width) * duration,
			});
	}
	...
	render() {
		const { width, height, id, waveformId, selection, loading, selectColor, hover, position } =
			this.state;
		...
		const containerStyle = {
			cursor: selection.zoomInActive
				? 'zoom-in'
				: selection.zoomOutActive
					? 'zoom-out'
					: selection.handleActive
						? 'ew-resize'
						: 'default',
		};
					...
					<div id={'waveform-selection' + id} className={'waveform-selection-container'}>
						...
						{this.handles.map((pos, idx) => (
							<div
								...
								onMouseMove={(e) =>
									this.handleSelectionHandle(e, pos.includes('left') ? 'left' : 'right')
								}
								onMouseDown={(e) =>
									this.handleSelectionHandle(e, pos.includes('left') ? 'left' : 'right')
								}
