...
class Controls extends Component {
	...
	componentDidMount() {
		...
		Global.engine.on('recordingprogress', (id, prog) => {
			if (!id) this.setState({ recordingProgress: prog.elapsed });
		});
	}
    ...
    */
	...
	render() {
		...
		return (
			<div id='controls-container' className={hovering ? 'hovering' : ''}>
				<div
					id={'controls-buttons'}
				...
				>
					<MdFiberManualRecord
						onClick={() => this.onRecord(!recording)}
						className={
							recordingProgress.recording
								? 'recording'
								: recordingProgress.processing
									? 'processing'
									: undefined
						}
					/>
					{playId !== undefined ? (
						<MdStop onClick={() => this.onStop()} />
					) : (
						<MdPlayArrow
							className={recordings.length ? 'control-toggle' : ''}
							onClick={() => this.onPlay()}
						/>
					)}
					<div id={'controls-recording-progress'}>
						{this.formatDuration(elapsed || recordingProgress.elapsed)}
					</div>
					<div id={'controls-recordings'} data-tip data-for={'tt-recordings'}>
						<MdQueueMusic
							data-tip
							data-for={'tt-recordings'}
							onClick={() => this.props.onToggleRecordings(!showRecordings)}
						/>
					...
					<MdRepeat
						data-tip
						data-for={'tt-loop'}
						onClick={() => this.onLoop(!looping)}
						className={looping ? 'control-toggle' : ''}
					/>
					<IoMdLock
						data-tip
						data-for={'tt-lock'}
						onClick={() => this.onLocked(!locked)}
						className={locked ? 'control-toggle' : ''}
					/>
					<MdFullscreen
						data-tip
						data-for={'tt-fs'}
						onClick={() => this.onFullscreen(!fullscreen)}
						className={fullscreen ? 'control-toggle' : ''}
					/>
					<GoSettings
						data-tip
						data-for={'tt-controls'}
						onClick={() => this.onControls(!controls)}
						className={controls ? 'control-toggle' : ''}
					/>
					<MdSettingsBackupRestore
						data-tip
						data-for={'tt-reset'}
					...
					<IoMdHelp
						data-tip
						data-for={'tt-help'}
						onClick={() => this.props.onToggleHelp(!showHelp)}
					/>
				...
				<div id={'controls-volume'}>
					<MdVolumeUp
						id='controls-volume-mute'
						data-tip
						data-for={'tt-mute'}
						onClick={() => this.onMute(!muted)}
						className={muted ? 'control-toggle' : ''}
					/>
					...
					{midiDevices.length > 0 ? (
						<Select
							...
							options={midiDevices.map((d) => {
								return { label: d.name, value: d.deviceId };
							})}
						...
						<Select
							value={0}
							options={[
								{ label: midiSupported ? 'MIDI not connected' : 'MIDI not supported', value: 0 },
							]}
						/>
					)}
